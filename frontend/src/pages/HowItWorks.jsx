...
function HowItWorks({ onBack, onTryDashboard, theme, onToggleTheme }) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-10 px-4 sm:px-6 lg:px-8 transition-colors">
        <div className="max-w-3xl mx-auto space-y-10">
          ...
          <header className="text-center space-y-3">
            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">How Credit Lens Works</h1>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-3 transition-colors">
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700 pb-2">The Big Picture</h2>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-4 transition-colors">
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700 pb-2">The 7 Signals Credit Lens Looks At</h2>
            <div className="grid gap-4 sm:grid-cols-2">
              {SIGNALS.map((s) => (
                <div key={s.name} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg border border-gray-100 dark:border-gray-700">
                  <h3 className="font-semibold text-gray-800 dark:text-gray-100 mb-1">{s.name}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">{s.summary}</p>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-4 transition-colors">
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700 pb-2">Understanding Your Results</h2>
            <div className="space-y-4">
              {OUTPUTS.map((o) => (
                <div key={o.name} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-4">
                  <span className="font-semibold text-gray-800 dark:text-gray-100 sm:w-56 shrink-0">{o.name}</span>
                  <span className="text-sm text-gray-600 dark:text-gray-300">{o.detail}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-4 transition-colors">
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700 pb-2">Glossary</h2>
            <dl className="space-y-4">
              {GLOSSARY.map((g) => (
                <div key={g.term}>
                  <dt className="font-semibold text-gray-800 dark:text-gray-100">{g.term}</dt>
                  <dd className="text-sm text-gray-600 dark:text-gray-300 mt-0.5">{g.def}</dd>
                </div>
              ))}
            </dl>
          </section>
